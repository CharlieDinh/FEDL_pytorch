...
def main(dataset, algorithm, model, batch_size, learning_rate, hyper_learning_rate, L, num_glob_iters,
    ...
    for i in range(times):
        ...
        # select algorithm
        if(algorithm == "FedAvg"):
            server = FedAvg(dataset, algorithm, model, batch_size, learning_rate, hyper_learning_rate, L, num_glob_iters, local_epochs, optimizer, clients_per_round, i)
        
        if(algorithm == "FEDL"):
            server = FEDL(dataset, algorithm, model, batch_size, learning_rate, hyper_learning_rate, L, num_glob_iters, local_epochs, optimizer, clients_per_round, i)
        server.train()
        server.test()

    # Average data 
    average_data(num_users=clients_per_round, loc_ep1=local_epochs, Numb_Glob_Iters=num_glob_iters, lamb=L,learning_rate=learning_rate, hyper_learning_rate = hyper_learning_rate, algorithms=algorithm, batch_size=batch_size, dataset=dataset,times = times)
...
if __name__ == "__main__":
    ...
    parser.add_argument("--hyper_learning_rate", type=float, default=4, help=" Learning rate of FEDL")
